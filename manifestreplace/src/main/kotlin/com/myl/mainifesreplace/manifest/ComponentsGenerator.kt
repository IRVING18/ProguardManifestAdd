    ...
    fun generateComponent(applicationID: String, manifestConfig: ManifestConfig): String {
        val application = xml("application") {
            for (i in 1..manifestConfig.activityCount) {
                "activity" {
                    attribute(ac_name, "${applicationID}.${ac_infix}AC$i${manifestConfig.prefix}")
                }
            }

            for (i in 1..manifestConfig.providerCount) {
                "provider" {
                    attribute(ac_name, "${applicationID}.${pd_infix}PD$i${manifestConfig.prefix}")
                    attribute(authorities, "${applicationID}.$pd_author$i${manifestConfig.prefix}")
            ...
            for (i in 1..manifestConfig.serviceCount) {
                "service" {
                    attribute(ac_name, "${applicationID}.${sc_infix}SC$i${manifestConfig.prefix}")
                }
            }

            for (i in 1..manifestConfig.receiverCount) {
                "receiver" {
                    attribute(ac_name, "${applicationID}.${rc_infix}RC$i${manifestConfig.prefix}")
                    attribute(exported, "false")
                    intentFilter {
                        action {
                            attribute(ac_name, "${applicationID}.${action}RC$i${manifestConfig.prefix}")
                        }
                    }
                }
            }
